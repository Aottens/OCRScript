#!/usr/bin/env python3
"""OCR extraction for CX Designer screenshots."""
...
class ExtractionConfig:
    min_symbols: int
    symbol_conf: float
    symbol_psm_modes: Tuple[int, ...]


DEFAULT_CONFIG = ExtractionConfig(min_symbols=2, symbol_conf=55.0, symbol_psm_modes=(11, 6))
...
def preprocess_symbol_image(image_bgr: np.ndarray) -> np.ndarray:
    gray = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)
    scaled = cv2.resize(gray, None, fx=2.0, fy=2.0, interpolation=cv2.INTER_CUBIC)
...
def ocr_roi(image: np.ndarray, roi: Tuple[int, int, int, int], psm: int) -> Tuple[str, float]:
    x, y, w, h = roi
    if w <= 0 or h <= 0:
        return "", 0.0
...
def run_extraction(
    ...
    for file_path in files:
        ...
        symbols_image = preprocess_symbol_image(image_bgr)
...
def main() -> int:
    ...
    parser.add_argument("--min_symbols", type=int, default=DEFAULT_CONFIG.min_symbols)
    parser.add_argument("--symbol_conf", type=float, default=DEFAULT_CONFIG.symbol_conf)
    args = parser.parse_args()

    config = ExtractionConfig(
        min_symbols=args.min_symbols,
        symbol_conf=args.symbol_conf,
        symbol_psm_modes=DEFAULT_CONFIG.symbol_psm_modes,
    )
